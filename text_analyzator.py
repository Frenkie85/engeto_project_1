...
delka_slov = dict()
for slovo in vycisteny_text:
    delka_slov.setdefault(slovo, len(slovo))
# print("Delka slov",delka_slov)

serazene = sorted(delka_slov.values(), reverse=False)[::]
# print("serazene", serazene)

vysledek = []
for slovo in delka_slov:
    if delka_slov[slovo] in serazene:
        vysledek.append((delka_slov[slovo], slovo))
hvezda = "*"
...
cetnost = dict()
for slovo in vycisteny_text:
    if slovo not in cetnost:
        cetnost[slovo] = 1
    else:
        cetnost[slovo] = cetnost[slovo] + 1

# print("CETNOST:", cetnost)


dict_delek = {}
for delka in serazene:
    if delka in dict_delek.keys():
        dict_delek[delka] += 1
    else:
        dict_delek[delka] = 1

# print("delky", dict_delek)

# Slova s nejcastesjim vyskytem
for x, y in dict_delek.items():
    print(f"{x}  |{(hvezda) * (y):<13} |{y}".format(x, y))
